...
words=json.load(open(sys.argv[1],encoding="utf-8")); out=sys.argv[2]; CY=int(sys.argv[3]); FS=int(sys.argv[4])
corr={}; drops=set(); repl={}; last_end=None
if len(sys.argv)>5 and sys.argv[5]!="-":
    c=json.load(open(sys.argv[5],encoding="utf-8")); corr={int(k):v for k,v in c.get("corr",{}).items()}; drops=set(c.get("drops",[])); repl=c.get("repl",{})
